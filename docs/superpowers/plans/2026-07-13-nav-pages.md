...
> Tokens verified against `:root` in `globals.css`: `--mono` (mono font), `--ink-500` (muted text), `--s3` (spacing). Mirrors the existing `.eyebrow` rule.
...
Open `http://localhost:3000`. Verify the nav shows `Home · Services · Team · Results · Insights · Contact · FAQ`, each item navigates to its page, the active item is highlighted, and the footer Company column lists Results/Insights/FAQ. Confirm "View Services" goes to `/services` and "See all FAQs" goes to `/faq`.
...
- Nav all top-level (`Home · Services · Team · Results · Insights · Contact · FAQ`) → Task 6. ✓
...
**Note:** `.card__meta` uses verified project tokens (`--mono`, `--ink-500`, `--s3`), matching the existing `.eyebrow` rule — no guesswork remains.
